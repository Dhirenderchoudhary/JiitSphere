...
const extractPhotoText = (value, depth = 0) => {
  ...
  for (const key of preferredKeys) {
    ...
    const matchedEntry = Object.entries(value).find(
      ([rawKey]) => normalizeToken(rawKey) === keyNorm
    );
...
export default function ProfileView({ token, onExpired }) {
  ...
  useEffect(() => {
    ...
    const rawName = findValue(['studentname', 'name', 'student_name'], ['student name', 'name']);

    const src = normalizePhotoSrc(rawPhoto);

    if (
      src &&
      !needsPortalPhotoProxy(src) &&
      !src.startsWith('blob:') &&
  ...
  if (profile?.realData === false) {
    return (
      <p className="pb-24 text-sm text-muted-foreground">
        {profile.message || message || 'No direct profile data available.'}
      </p>
    );
  }
  ...
  const profileName =
    findProfileValue(['studentname', 'name'], ['student name']) || profile.studentname;

  const rows = [
    ['Name', profileName],
    [
      'Enrollment',
      (profile.enrollmentno && String(profile.enrollmentno).length > 4
  ...
  const extraRows = flattenScalarPairs(profile || {})
    ...
    .filter(([key, value]) => {
      ...
      if (/(token|jwt|cookie|session|captcha|checksum|hash|photoevent|eventcode|raw)/.test(keyText))
        return false;
  ...
  const personalRows = [
    ...
    [
      ...
      profile.dateofbirth ||
        profile.dob ||
        findProfileValue(['birthdate', 'studentdob'], ['date of birth', 'dob']),
    ],
    ...
    [
      ...
      profile.fathersname ||
        profile.fathername ||
        findProfileValue(['fathername'], ['father name']),
    ],
    [
      'Mother Name',
      profile.mothersname ||
        profile.mothername ||
        findProfileValue(['mothername'], ['mother name']),
    ],
  ];
  ...
  const contactRows = [
    [
      ...
      profile.instituteemail ||
        ...
        profile.emailid ||
        findProfileValue([], ['institute email']),
    ],
    [
      'Personal Email',
      profile.personalemail ||
        profile.studentpersonalemailid ||
        profile.parentemailid ||
        profile.fatheremailid ||
        profile.guardianemailid ||
        findProfileValue([], ['personal email', 'parent email']),
    ],
    [
      'Mobile',
      profile.mobile ||
        profile.studentcellno ||
        profile.studentmobileno ||
        profile.contactno ||
        findProfileValue([], ['mobile', 'cell no']),
    ],
    [
      'Alternate Contact',
      profile.alternatecontact ||
        profile.parentmobileno ||
        profile.fathermobileno ||
        profile.guardianmobileno ||
        profile.parentcellno ||
        findProfileValue([], ['alternate contact', 'guardian mobile']),
    ],
    [
      'Telephone',
      profile.studenttelephoneno ||
        profile.parenttelephoneno ||
        findProfileValue([], ['telephone', 'phone']),
    ],
        ...
        <div className="space-y-6 p-6 sm:p-8">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-sm p-6 sm:p-8 flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8 group">
            ...
            <div className="text-center sm:text-left flex-1 min-w-0 z-10 w-full mt-2 sm:mt-0">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-2">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/80 mb-1.5">
                    Corporate Access Identity
                  </p>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {visibleRows.map(([k, v]) => (
              <div
                key={k}
                className="rounded-xl border border-border/40 p-4 space-y-1 hover:border-primary/20 transition-colors"
              >
            ...
            <div className="mt-8 space-y-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {extraRows.map(([k, v]) => (
                  <div
                    key={`extra-${k}`}
                    className="rounded-lg border-l-2 border-primary/20 bg-secondary/10 px-3 py-2 text-xs"
                  >
