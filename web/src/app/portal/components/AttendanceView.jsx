  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div className="absolute top-[50%] flex flex-col items-center">
        <span className="text-2xl font-black font-[var(--font-instrument-sans)] tracking-tighter text-foreground">
          {Math.round(pct)}%
        </span>
        <span className="text-[9px] font-bold text-muted-foreground mt-0.5">
          {safe ? "It's already great!" : 'Needs attention!'}
        </span>
      </div>
...
export default function AttendanceView({ token, onExpired, setCustomSidebar }) {
  ...
  const selectedSemesterLabel = useMemo(() => {
    ...
    const current = semesters.find(
      (sem) => String(sem?.registration_id || '') === String(selectedSem || '')
    );
      ...
      try {
        ...
        setSubjectCounts((prev) => {
          ...
          for (const row of rows) {
            ...
            if (
              next[subjectCode] &&
              next[subjectCode].source !== 'pending' &&
              !next[subjectCode].loading
            ) {
              continue;
            }
            ...
            if (trustedRatio?.total) {
              next[subjectCode] = {
                attended: Number(trustedRatio.attended),
                total: Number(trustedRatio.total),
                loading: false,
                source: trustedRatio.source,
              };
            } else {
        ...
        if (!initialSemesterFallbackDone.current) {
          ...
          const alternatives = semRows.filter(
            (sem) => sem?.registration_id && sem.registration_id !== selectedSem
          );
          if (alternatives.length) {
            const altResponses = await Promise.all(
              alternatives.map(async (s) => ({
                sem: s,
                rows:
                  (await fetchPortalAttendance(token, s.registration_id).catch(() => ({})))?.data
        ...
        resetUI();
        setAttendance([]);
        setMessage(err?.message || 'Data sync error.');
      } finally {
      ...
      try {
        ...
        setSubjectCounts((prev) => {
          ...
          Object.keys(counts).forEach((code) => {
            next[code] = {
              attended: Number(counts[code].attended),
              total: Number(counts[code].total),
              loading: false,
              source: counts[code].source,
            };
          });
  ...
  const aggregateMetrics = useMemo(() => {
    ...
    attendance.forEach((row) => {
      ...
      if (subjectCounts[code] && subjectCounts[code].total > 0) {
        tC += subjectCounts[code].total;
        tA += subjectCounts[code].attended;
      } else {
  ...
  const activeCode = isDetailView
    ? String(activeSubject?.subjectcode || activeSubject?.individualsubjectcode || '').trim()
    : null;
          ...
          {attendance.map((row) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'w-full text-left py-3.5 px-4 rounded-xl transition-all mb-1 mt-1',
                  isSelected
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-1.5">
                  ...
                  >
                    {row.subjectdesc || subjectCode}
                  </span>
                  <span
                    className={cn(
                      'text-[11px] font-black shrink-0 mt-0.5',
                      pct >= targetVal ? 'text-emerald-500' : 'text-rose-500'
                    )}
                  >
                    {Math.round(pct)}%
                  </span>
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[9px] font-mono tracking-wider opacity-60 bg-foreground/5 px-1.5 py-0.5 rounded uppercase">
                    {subjectCode}
                  </span>
                  <span className="text-[10px] font-bold text-muted-foreground">
                    {countState && !countState.loading
                      ? `${countState.attended} / ${countState.total}`
                      : (() => {
                          const rc = rowCounts(row);
                          return rc.total > 0 ? (
                            `${rc.attended} / ${rc.total}`
                          ) : (
  ...
  if (isDetailView) {
    ...
    const ratioParam =
      layoutTotal > 0 ? { attended: layoutAttended, total: layoutTotal } : undefined;
  ...
  const performanceCard = (
    <motion.div
      ...
      className="bg-card border border-border shadow-sm rounded-2xl p-4 sm:p-5 shrink-0 flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-stretch"
    >
      ...
      <div className="flex flex-col items-center justify-center shrink-0 min-w-[200px]">
        ...
        {aggregateMetrics || isDetailView ? (
          ...
          <div className="h-[120px] flex items-center justify-center text-xs font-mono uppercase tracking-widest text-muted-foreground animate-pulse">
            Computing metrics...
          </div>
        ) : (
          <div className="h-[120px] flex items-center justify-center text-xs font-medium text-muted-foreground">
        ...
        <div className="flex-1 flex flex-row items-center w-full border-t border-border/40 pt-4 md:border-t-0 md:pt-0 md:border-l md:border-border md:pl-6 gap-4 sm:gap-6 md:gap-10">
          ...
          <div className="flex-1 flex flex-col justify-center h-full">
            <div className="flex items-center gap-1.5 text-muted-foreground mb-1.5">
              ...
              {layoutSafe ? (
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 ml-auto hidden sm:block" />
              ) : (
                <AlertCircle className="w-3.5 h-3.5 text-rose-500 ml-auto hidden sm:block" />
              )}
            </div>
            <span
              className={cn(
                'text-xl md:text-2xl font-black tracking-tighter leading-none mb-1.5',
                layoutSafe ? 'text-emerald-500' : 'text-rose-500'
              )}
            >
            ...
            {aggregateMetrics?.fallback ? (
              <>
                ...
                <span className="text-[9px] font-bold text-muted-foreground uppercase">
                  Avg {Math.round(aggregateMetrics.pct)}%
                </span>
              </>
            ) : (
              <>
                <div className="flex items-baseline gap-1 mb-1.5">
                  {layoutTotal > 0 ||
                  (isDetailView &&
                    subjectCounts[activeCode] &&
                    !subjectCounts[activeCode].loading) ? (
                    <>
                      <span className="text-xl md:text-2xl font-black tracking-tighter text-foreground leading-none">
                        {layoutAttended}
                      </span>
                ...
                <div className="flex flex-col sm:flex-row gap-1 sm:gap-3 text-[9px] font-bold text-muted-foreground uppercase leading-tight">
                  <span>
                    Lec{' '}
                    {isDetailView && (
                      <span className="text-foreground tracking-wider ml-0.5">
                        {toPercent(activeSubject?.Lpercentage)}
                      </span>
                    )}
                  </span>
                  <span>
                    ...
                    {isDetailView && (
                      <span className="text-foreground tracking-wider ml-0.5">
                        {toPercent(activeSubject?.Tpercentage)}
                      </span>
                    )}
                  </span>
                  <span>
                    ...
                    {isDetailView && (
                      <span className="text-foreground tracking-wider ml-0.5">
                        {toPercent(activeSubject?.Ppercentage)}
                      </span>
                    )}
                  </span>
                </div>
    ...
    <div className="bg-card border border-border shadow-sm rounded-2xl flex flex-col w-full flex-1 min-h-0 overflow-hidden">
      ...
      <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-border flex items-center justify-between bg-muted/10 shrink-0">
        ...
        <span className="text-[10px] sm:text-xs font-semibold text-muted-foreground shrink-0">
          {isDetailView && historyDetail?.rows?.length} Records
        </span>
      </div>
      ...
      <div className="flex-1 overflow-auto custom-scrollbar p-0 sm:p-6 relative">
        {isDetailView ? (
          historyDetail?.loading ? (
            <div className="flex items-center justify-center h-full text-xs font-mono uppercase tracking-widest text-muted-foreground">
              Fetching records...
            </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {historyDetail.rows.map((row, i) => (
                  ...
                  >
                    <td className="px-4 py-3.5 font-bold text-foreground w-[160px]">
                      {row.datetime || '-'}
                    </td>
                    <td className="px-4 py-3.5 text-muted-foreground w-[100px]">
                      {row.classtype || 'Class'}
                    </td>
                    <td className="px-4 py-3.5 text-muted-foreground max-w-full break-words">
                      {row.topic || '-'}
                    </td>
            ...
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground space-y-3">
              ...
              <p className="text-sm font-medium text-center max-w-xs">
                {historyDetail?.error ||
                  'No class-by-class records available for this subject yet.'}
              </p>
            </div>
          )
        ) : loading ? (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-xs font-bold text-foreground leading-snug line-clamp-2">
                      {row.subjectdesc || code}
                    </span>
                    <span
                      className={cn(
                        'text-lg font-black tracking-tighter shrink-0',
                        safe ? 'text-emerald-500' : 'text-rose-500'
                      )}
                    >
                      {Math.round(pct)}%
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-mono tracking-wider text-muted-foreground/60 uppercase">
                      {code}
                    </span>
                    <span className="text-[10px] font-bold text-muted-foreground">
                      {countState && !countState.loading
          ...
          <div className="flex justify-between items-start gap-4">
            <div>
              ...
              <p className="text-xs font-medium text-muted-foreground mt-0.5">
                Select a module to view timeline
              </p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
          {attendance.map((row) => {
            ...
            return (
              <button
                key={subjectCode}
                onClick={() => selectSubject(row, true)}
                className={cn(
                  'w-full text-left py-3.5 px-4 rounded-xl transition-all mb-1',
                  isSelected
                    ? 'bg-primary/5 text-primary border border-primary/10'
                    : 'bg-transparent hover:bg-muted/50 text-muted-foreground border border-transparent'
                )}
      ...
      <Drawer open={isMobileDrawerOpen} onOpenChange={setIsMobileDrawerOpen}>
        <DrawerContent className="flex flex-col pt-2 bg-background border-t border-border">
          <DrawerHeader className="pb-2 pt-0 shrink-0">
            <DrawerTitle className="text-lg font-black text-left">
