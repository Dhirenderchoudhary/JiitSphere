...
export default function FeesView({ token, semesters = [], onExpired }) {
  ...
  const loadFees = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        if (!feeRows.length) {
          setDebugHint(
            'No semester-wise fee rows were returned by portal for this account right now.'
          );
        } else {
          setDebugHint('');
        }
  ...
  const groupedFees = useMemo(() => {
    ...
    for (const item of sortedFees) {
      ...
      const semesterCode =
        semesterCodeFromMeta || item?.registration_code || registrationId || 'Semester';
    ...
    return Array.from(bySemester.values())
      ...
      .sort(
        (a, b) =>
          semesterSortScore(b?.registration_code, b?.semester_label) -
          semesterSortScore(a?.registration_code, a?.semester_label)
      );
    ...
    <div className="space-y-6 pb-24 sm:pb-20">
      <div className="rounded-2xl border border-border/40 bg-card shadow-sm">
        <div className="p-6 pb-4 border-b border-border/20">
          <h3 className="text-lg font-bold text-foreground font-[var(--font-instrument-sans)]">
            Wallet Summary
          </h3>
          <p className="text-xs font-medium text-muted-foreground">
            Consolidated financial standing across all terms
          </p>
        </div>
        <div className="p-6">
        ...
        {!groupedFees.length ? (
          <div className="rounded-2xl p-12 text-center border border-dashed border-border/30">
            <p className="text-sm font-medium text-muted-foreground whitespace-pre-line">
              {message || debugHint || 'No financial records found.'}
            </p>
          </div>
        ) : (
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="rounded-xl border border-border/40 p-3 space-y-1">
                      ...
                      <p className="text-sm font-bold text-foreground">
                        {formatCurrency(item.total_demand)}
                      </p>
                    </div>
