      ...
      <Card className="rounded-xl border-emerald-500/30 bg-emerald-500/5 spotlight-card shadow-xl overflow-hidden">
        <CardHeader className="pb-4 border-b border-emerald-500/20 bg-emerald-500/5">
          ...
          <CardDescription className="text-[10px] font-bold uppercase tracking-widest opacity-60">
            Verified student access telemetry for Study Vault
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6">
          {studyAnalytics ? (
            <div className="space-y-6">
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div className="border border-emerald-500/20 p-4 bg-background/40">
                  <p className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-600/60 mb-1">
                    TOTAL AUTHORIZATIONS
                  </p>
                  <p className="text-3xl font-black text-emerald-600 font-[var(--font-instrument-sans)] leading-none">
                    {compact(studyAnalytics.totalSignIns)}
                  </p>
                </div>
                <div className="border border-emerald-500/20 p-4 bg-background/40">
                  <p className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-600/60 mb-1">
                ...
                <div className="border border-emerald-500/20 p-4 bg-background/40 sm:col-span-2">
                  <p className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-600/60 mb-3">
                    ACTIVITY SPECTRUM // 7D
                  </p>
                  <div className="grid grid-cols-7 gap-1.5 h-12">
                    {(studyAnalytics.last7Days || []).map((day) => (
                      <div key={day.date} className="relative group flex items-end h-full">
                        ...
                        >
                          {day.count > 0 && (
                            <div className="absolute inset-0 bg-emerald-400 group-hover:block hidden animate-pulse" />
                          )}
                        </div>
                      </div>
                    ))}
            ...
            <div className="p-12 text-center border border-dashed border-emerald-500/20">
              <p className="text-xs font-medium text-emerald-600/40 uppercase tracking-widest">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xl font-black text-foreground">
                  {compact(analytics.totalRequests)}
                </p>
              </div>
              <div className="space-y-1">
                <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">
                ...
                <p className="text-xl font-black text-foreground">
                  {compact(analytics.uniqueVisitors)}
                </p>
              </div>
            </div>
            <div className="pt-4 border-t border-border/5 space-y-2">
              <div className="flex justify-between items-center text-[10px]">
                ...
                <span className="font-black text-primary">
                  {compact(analytics.p95ResponseTimeMs)}ms
                </span>
              </div>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-2 gap-2">
              {statusBreakdown.map((item) => (
                <div
                  key={item.label}
                  className="border border-border/40 p-3 bg-muted/5 flex flex-col justify-between"
                >
        ...
        <CardContent className="p-0">
          <div className="max-h-[400px] overflow-y-auto overflow-x-hidden border-b border-border/10">
            {(analytics.recentRequests || []).slice(0, 30).map((row, idx) => (
              <div
                key={`${row.at}-${idx}`}
                className="grid grid-cols-[80px_1fr_120px] items-center gap-4 px-6 py-4 border-b border-border/5 hover:bg-primary/5 transition-colors group"
              >
                <div
                  className={cn(
