...
export default function ExamsView({ token, semesters = [], onExpired }) {
  ...
  const loadExams = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        if (!rows.length) {
          setMessage(
            'No exam schedule available right now. Try refresh once the portal updates data.'
          );
        }
      } catch (err) {
  ...
  const filteredExams = useMemo(() => {
    ...
    const selectedSemMeta = (semesters || []).find(
    ...
    return exams.filter((exam) => {
      ...
      const byCode =
        selectedCode && String(exam.registration_code || '').toLowerCase() === selectedCode;
      return byId || byCode;
    });
  ...
  return (
    <div className="space-y-4 pb-28 sm:pb-24">
      <div className={`p-4 ${glassPanel}`}>
        ...
        {lastSyncAt ? (
          <p className="mt-3 text-xs font-medium text-muted-foreground/50 text-right">
            Synced at {lastSyncAt}
          </p>
        ) : null}
      </div>
      ...
      {loading && !sortedExams.length ? (
        <div className="grid gap-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="rounded-2xl border border-border/40 bg-card p-6 min-h-[140px] flex items-center justify-center"
            >
        ...
        <div className="rounded-2xl p-12 text-center border border-dashed border-border/30">
          <p className="text-sm font-medium text-muted-foreground">
            {message || 'No scheduled exams found.'}
          </p>
        </div>
      ) : null}
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-border/20 pt-4">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-sm font-bold text-foreground">
                        {toExamDateLabel(exam.date) || 'Pending'}
                      </p>
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[9px] font-medium text-muted-foreground">
