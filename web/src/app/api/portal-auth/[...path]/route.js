...
const PORTAL_REQUEST_TIMEOUT_MS = Number(process.env.NEXT_PUBLIC_API_TIMEOUT_MS || 20000);
const PORTAL_REQUEST_RETRIES = Math.max(0, Number(process.env.NEXT_PUBLIC_API_RETRIES || 1));
...
const joinPath = (segments = []) => `/${segments.map((segment) => String(segment || '').replace(/^\/+|\/+$/g, '')).filter(Boolean).join('/')}`;
...
const fetchWithRetry = async (targetUrl, options = {}) => {
  ...
  while (true) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), Math.max(3000, PORTAL_REQUEST_TIMEOUT_MS));
    ...
    try {
      ...
      if (attempt >= PORTAL_REQUEST_RETRIES) {
        throw error;
      }
      attempt += 1;
    } finally {
...
const proxyRequest = async (request, { params }) => {
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        message: 'Portal service is temporarily unavailable. Please retry.'
      },
      { status: 502 }
    );
  }
